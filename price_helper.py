...
def validate_and_extract(html, name):
    ...
    # Extract: find "Ungraded" then the first $X.XX within the window
    # after it -- matches PriceCharting's summary comparison table, not
    # the sold-listings history further down the page. Real HTML has
    # substantial markup (table headers for Grade 7/8/9/9.5/PSA10, CSS
    # classes) between the label and its value, so the window needs to
    # be generous -- a live test at 300 chars found nothing on any card;
    # this wider window is calibrated from that real failure, not a guess.
    idx = html.find("Ungraded")
    if idx == -1:
        return None, "no 'Ungraded' price table found"
    window = html[idx:idx + 4000]
    m = re.search(r">\s*\$([\d,]+\.\d{2})", window)
    if not m:
        # fall back to a looser match in case the exact markup differs
